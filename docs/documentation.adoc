...
decoded header section, the payload section containing e,g, username, current group and roles, and the method how the signature is created, representing the last part of the token. These three parts of the JWT are separated 
by a dot in the encoded version.

[INFO]
The value of <SHARED_SECRET> must identical for all services trusting the KIT Data Manager Authentication Service in order to be able to validate the signature. It should be a random string 
...
Finally, we want to show how to delete a group, or better said how to disable a group.
